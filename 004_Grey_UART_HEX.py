    ...
    while 1:
        ...
        if msglen:
            ...
            if "Usart End" in utf8_msg:
                ...
                # 发送数据
                log.info("uartread msg: {}  len: {}".format(ubinascii.hexlify(utf8_msg), msglen))
                # uart.write("{}".format(utf8_msg))
        else:
            utime.sleep_ms(1)
            continue
    state = 0
...
if __name__ == "__main__":
    ...
    uart = UART(UART.UART2, 57600, 8, 0, 1, 0)  # EC600S/EC600N使用
    ...
    # utime.sleep_ms(50)
    # uart.write("{}".format('\xab''\xba''\x03''\x00''\xa2''\xa5''\x00''\x00'))

    utime.sleep_ms(500)
    command_1 = [0xAB, 0xBA, 0x03, 0x00, 0xA2, 0xA5, 0x00, 0x00]
    bytearray_command_1 = bytearray(command_1)
    write_command_len = uart.write(bytearray_command_1)
